...
fun getUserIndex(users: List<User>) : Int {
    var index : Int?
...
fun listUsersByAge (users: List<User>) {
    var age : Int?

    do {
        print("Ingrese la edad para filtrar: ")
        age = readlnOrNull()?.toIntOrNull()
    ...
    val filteredUsers = users.filter { user -> user.getAge() > age }
    ...
    do {
        ...
        if (option == null) {
            ...
            when (option) {
                ...
                4 -> if (users.isEmpty()) println("No hay usuarios registrados.") else showUser(users.toList())

                5 -> if (users.isEmpty()) println("No hay usuarios registrados.") else listUsersByAge(users.toList())
